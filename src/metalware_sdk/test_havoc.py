...
def pretty_memory_config(memory_config: MemoryConfig) -> str:
  ...
  for memory in memory_config.memory_layout:
    ...
    result += f"{memory.base_addr} {memory.size} {memory.memory_type} {memory.file}\n"
  return result
...
class TestHavoc(TestCase):
    def test_alias_elf(self):
        ...
        client.create_project("alias-test", project_config, is_overwritable_temp=True)

        image_config = ImageConfig(image_arch=ImageArch.CORTEX_M, image_format=ImageFormat(elf=file_metadata.hash))
        client.create_image(project_name="alias-test", image_name="default", image_config=image_config)

        client.start_run(project_name="alias-test", config=RunConfig(image_name="default", dry_run=True))
    ...
    def test_zephyr_rom_infer(self):
        ...
        project_config = ProjectConfig(memory_config)
        client.create_project("zephyr-10064", project_config, is_overwritable_temp=True)
        ...
        client.create_image(project_name="zephyr-10064", image_name="default", image_config=image_config)

        client.start_run(project_name="zephyr-10064", config=RunConfig(image_name="default", dry_run=True))
    ...
    # TODO: test non-temp project
    # TODO: test project settings update
